...
void build_connectivity(Mesh &hmi) {
	hmi.Es.clear();
	if (hmi.Hs.size() && hmi.type != Mesh_type::Hyb) hmi.Fs.clear();
	//either hex or tri
		...
		for (uint32_t i = 0; i < tempF.size(); ++i) {
			if (i == 0 || (i != 0 &&
						   (std::get<0>(tempF[i]) != std::get<0>(tempF[i - 1]) ||
							std::get<1>(tempF[i]) != std::get<1>(tempF[i - 1]) ||
							std::get<2>(tempF[i]) != std::get<2>(tempF[i - 1]) ||
							std::get<3>(tempF[i]) != std::get<3>(tempF[i - 1])))) {
				f.id = F_num;
				F_num++;
...
bool disk_polygon(Mesh &mesh, Frame &frame, vector<vector<uint32_t>> &fes, vector<short> &E_flag, vector<short> &V_flag,
				  const bool &Ismesh) {
	vector<uint32_t> pes;
...
bool sphere_polyhedral(Mesh &mesh, Frame &frame, vector<vector<uint32_t>> &F_nvs, vector<vector<uint32_t>> &pfs,
					   vector<bool> &F_flag, vector<short> &E_flag, vector<short> &V_flag, const bool &Ismesh) {
...
void re_indexing_connectivity(Mesh &hmi, vector<bool> &H_flag, Mesh &Ho, vector<int32_t> &V_map,
							  vector<int32_t> &V_map_reverse, vector<int32_t> &H_map, vector<int32_t> &H_map_reverse) {
	V_map.clear();
	V_map_reverse.clear();
	H_map.clear();
	H_map_reverse.clear();
...
void re_indexing_connectivity_sur(Mesh &mi, vector<bool> &f_flag, Mesh &mo, vector<int32_t> &V_map,
								  vector<int32_t> &V_map_reverse, vector<int32_t> &f_map,
								  vector<int32_t> &f_map_reverse) {
	V_map.clear();
	V_map_reverse.clear();
	f_map.clear();
	f_map_reverse.clear();
...
void move_boundary_vertices_back(Mesh_Domain &md, std::vector<int> &hex2Octree_map) {
	...
	re_indexing_connectivity(md.mesh_entire, md.H_flag, md.mesh_subA, md.V_map, md.V_map_reverse, md.H_map,
							 md.H_map_reverse);
}
...
void extract_surface_conforming_mesh(Mesh &meshi, Mesh &mesho, vector<int32_t> &V_map, vector<int32_t> &V_map_reverse,
									 vector<int32_t> &F_map, vector<int32_t> &F_map_reverse) {
	mesho.Vs.clear();
	mesho.Es.clear();
	mesho.Fs.clear();
	mesho.Hs.clear();
...
void orient_surface_mesh(Mesh &hmi) {
	...
	while (!pf_temp.empty()) {
		...
		for (auto eid : hmi.Fs[fid].es)
			for (auto nfid : hmi.Es[eid].neighbor_fs) {
				...
				int32_t v0_pos_ =
						std::find(hmi.Fs[nfid].vs.begin(), hmi.Fs[nfid].vs.end(), v0) - hmi.Fs[nfid].vs.begin();
				int32_t v1_pos_ =
						std::find(hmi.Fs[nfid].vs.begin(), hmi.Fs[nfid].vs.end(), v1) - hmi.Fs[nfid].vs.begin();
	...
	for (auto f : hmi.Fs) {
		...
		Vector3d center;
		center.setZero();
		for (auto vid : fvs) center += hmi.V.col(vid);
		center /= fvs.size();
...
void direction_correction(std::vector<std::vector<int>> &loops, Mesh &m) {
	...
	for (int i = 0; i < loops.size(); i++) {
		...
		for (int j = 0; j < loops[i].size(); j++) {
			int v_0 = loops[i][(j - 1 + loops[i].size()) % loops[i].size()], v0 = loops[i][j], v1 = loops[i][(j + 1) %
																											 loops[i].size()];
...
void face_soup_info(Mesh &hmi, vector<uint32_t> &fs_soup, vector<bool> &E_tag, vector<uint32_t> &bes,
					vector<uint32_t> &bvs) {
	bes.clear();
	int f = -1;
	for (auto fid : fs_soup)
...
void face_soup_roll_region(Mesh &hmi, uint32_t sfid, vector<bool> &E_tag, vector<uint32_t> &bes, vector<uint32_t> &fs,
						   vector<uint32_t> &bvs) {
...
void face_soup_expand_region(Mesh &hmi, vector<uint32_t> &fs_soup, vector<bool> &E_tag, vector<bool> &F_tag,
							 vector<uint32_t> &bes) {
	for (auto eid : bes)E_tag[eid] = true;
...
void DijkstraComputePaths(vector<vertex_t> &source, const adjacency_list_t &adjacency_list,
...
bool triangle_mesh_feature(Mesh_Feature &mf) {
	...
	for (uint32_t i = 0; i < mesh.Es.size(); i++) {
		...
		std::function<bool(uint32_t, uint32_t, uint32_t &)> feature_line_proceed = [&](uint32_t vid, uint32_t eid,
																					   uint32_t &neid) -> bool {
...
bool project_surface_update_feature(Mesh_Feature &mf, Feature_Constraints &fc, MatrixXd &V, VectorXi &b, MatrixXd &bc,
									uint32_t Loop) {
...
//						bool found = phong_projection(tids, Loop, tid, v, interpolP, interpolN, PreinterpolP,
//													  PreinterpolN);
//
...
//							if (phong_projection(tids, Loop, tid_temp, v, interpolP, interpolN, PreinterpolP,
//												 PreinterpolN))
//								tid = tid_temp;
//							else {
...
bool phong_projection(vector<uint32_t> &tids, uint32_t Loop, uint32_t &tid, Vector3d &v, Vector3d &interpolP,
					  Vector3d &interpolN, Vector3d &PreinterpolP, Vector3d &PreinterpolN) {
	...
	while (ts.size()) {
		...
		for (uint32_t j = 0; j < ts.size(); j++) {
			...
			if ((uv.x() >= 0.0 || num_equal(uv.x(), 0.0, Precision_Pro)) &&
				(uv.y() >= 0.0 || num_equal(uv.y(), 0.0, Precision_Pro)) &&
...
void projectPointOnQuad(const vector<Vector3d> &quad_vs, vector<Vector3d> &vs_normals, const Vector3d &p, Vector2d &uv,
						Vector3d &interpolP, Vector3d &interpolN) {
	...
	for (int i = 0; i < 4; ++i) {
		...
		Vector3d dPdu =
				(1 - uv.y()) * quad_vs[1] + uv.y() * quad_vs[2] - ((1 - uv.y()) * quad_vs[0] + uv.y() * quad_vs[3]);
		Vector3d dPdv =
				(1 - uv.x()) * quad_vs[3] + uv.x() * quad_vs[2] - ((1 - uv.x()) * quad_vs[0] + uv.x() * quad_vs[1]);
		Vector3d dNdu = (1 - uv.y()) * vs_normals[1] + uv.y() * vs_normals[2] -
						((1 - uv.y()) * vs_normals[0] + uv.y() * vs_normals[3]);
		Vector3d dNdv = (1 - uv.x()) * vs_normals[3] + uv.x() * vs_normals[2] -
						((1 - uv.x()) * vs_normals[0] + uv.x() * vs_normals[1]);
...
void projectPointOnTriangle(const vector<Vector3d> &tri_vs, const vector<Vector3d> &vs_normals, const Vector3d &p,
							Vector2d &uv, Vector3d &interpolP, Vector3d &interpolN) {
	Eigen::Matrix<double, 3, 2> jacobian;
...
void
compute_referenceMesh(MatrixXd &V, vector<Hybrid_F> &F, vector<uint32_t> &Fs, vector<MatrixXd> &Vout, bool square) {
...
void compute_referenceMesh(MatrixXd &V, vector<Hybrid> &H, vector<bool> &H_flag, vector<uint32_t> &Hs,
						   vector<MatrixXd> &Vout, bool cube) {
	vector<MatrixXd>().swap(Vout);
...
void compute_referenceMesh(MatrixXd &V, vector<Hybrid> &H, vector<uint32_t> &Hs, vector<MatrixXd> &Vout, bool cube,
						   vector<double> &Vols) {
	vector<MatrixXd>().swap(Vout);
