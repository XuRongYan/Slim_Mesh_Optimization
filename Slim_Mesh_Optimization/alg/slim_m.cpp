		...
		for (int i = 0; i < s.Ji.rows(); ++i) {
			...
			switch (s.slim_energy) {
				...
				case SLIM_ENERGY::EXP_SYMMETRIC_DIRICHLET: {
					...
					double in_exp =
							exp_f * (pow(s1, 2) + pow(s1, -2) + pow(s2, 2) + pow(s2, -2) + pow(s3, 2) + pow(s3, -2));
					double exp_thing = exp(in_exp);
				...
				case SLIM_ENERGY::EXP_CONFORMAL: {
					...
					double in_exp =
							exp_f * ((pow(s1, 2) + pow(s2, 2) + pow(s3, 2)) / (3 * pow((s1 * s2 * s3), 2. / 3)));;
					double exp_thing = exp(in_exp);
...
void solve_weighted_arap(SLIMData &s,
	...
	if (s.ts.known_value_post) {
		int N = s.dim * s.ts.post_index + s.ts.fc.b.rows();
		...
		for (int j = 0; j < s.ts.fc.b.rows(); j++) {
		...
		Aibi = L_ib * X_b;
		for (int m = 0; m < s.dim; m++)
			b_i.segment(m * s.ts.post_index, 1 * s.ts.post_index) =
					s.rhs.segment(m * s.v_n, s.ts.post_index) - Aibi.segment(m * s.ts.post_index, 1 * s.ts.post_index);
		b_i.segment(s.dim * s.ts.post_index, s.ts.fc.b.rows()) =
				s.rhs.segment(s.dim * s.v_n, s.ts.fc.b.rows()) -
				Aibi.segment(s.dim * s.ts.post_index, s.ts.fc.b.rows());
		...
		if (s.dim == 2) {
			...
			uint32_t Num_variables = s.ts.post_index * s.dim + s.ts.fc.b.rows();

			Eigen::VectorXd guess(Num_variables);
			for (int i = 0; i < s.ts.post_index; i++)
				for (int j = 0; j < s.dim; j++)
					guess(s.ts.post_index * j + i) = uv(i, j); // flatten vector
			//if (!s.ts.projection)
...
void pre_calc(SLIMData &s) {
	if (!s.has_pre_calc) {
		...
		if (s.F.cols() == 3) {
			...
			if (s.mesh_type == Mesh_type::Hex) {
				igl::grad_ref(s.V, s.F, s.ts.RT, G, true);
			} else {
				cout << s.V.row(s.F(0,2)) << endl;
				igl::grad(s.V, s.F, G, true);
			}
	...
	if (s.dim == 2) {
		...
		for (int i = 0; i < s.f_n; i++) {
			...
			switch (s.slim_energy) {
				...
				case SLIM_ENERGY::SYMMETRIC_DIRICHLET: {
					energy +=
							areas(i) * (pow(s1, 2) + pow(s1, -2) + pow(s2, 2) + pow(s2, -2) + pow(s3, 2) + pow(s3, -2));
					break;
				}
				case SLIM_ENERGY::EXP_SYMMETRIC_DIRICHLET: {
					energy += areas(i) * exp(s.exp_factor *
											 (pow(s1, 2) + pow(s1, -2) + pow(s2, 2) + pow(s2, -2) + pow(s3, 2) +
											  pow(s3, -2)));
					break;
				}
				case SLIM_ENERGY::LOG_ARAP: {
...
//	data.a_L.resize(data.ids_L.rows());
//	data.a_L.setZero();
////region
...
Eigen::MatrixXd slim_solve2(SLIMData &data, int iter_num, int type, bool verbose) {
	if (iter_num == -1) {
		double ratio = 1;
		int iter = 0;
		double energy_pre = -1, energy_cur = -1;
		int max_iter = 50;
		double MESHRATIO = 1;
		...
		for (uint32_t i = 0; i < data.ts.m.Hs.size(); i++)
			Hids.push_back(i);
		...
		do {
			...
			if (verbose) {
				...
				cout << "energys: " << data.engery_quality << " :" << data.engery_soft << " " << data.ts.lamda_glue
					 << endl;
			}
			iter++;
			...
			} else if (type == 1)//gluing
			{
				...
				data.ts.lamda_glue = std::min(LAMDA_GLUE_MIN_BOUND *
											  std::max(data.engery_quality / data.engery_soft * data.ts.lamda_glue,
													   1.0), LAMDA_GLUE_MAX_BOUND);
			}
		...
		for (int i = 0; i < iter_num; i++) {
			...
			if (fabs(old_energy - data.energy) < 1e-6) {
				cout << "converged" << endl;
				break;
			}
		}
